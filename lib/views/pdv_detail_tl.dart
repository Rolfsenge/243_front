...
class PdvDetailTl extends StatefulWidget {
  final Color color;
  ...
  @override
  State<PdvDetailTl> createState() => _PdvDetailTlState();
}

class _PdvDetailTlState extends State<PdvDetailTl> {
  bool loading = true;
  List<dynamic> sites = [];
  ...
  Future<void> fetchPDVSite() async {
    try {
      final url = Uri.parse(API.getPdvBySiteTl);
      final response = await http.post(
        url,
        body: {"statut_pdv": widget.statut_pdv, "tl_phone": widget.tl_phone},
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);

        print(widget.tl_phone);
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                buildInfoItem("Moy. jour", "${_arrondir(pdv['moy_jour'])} \$"),
              ],
            ),
