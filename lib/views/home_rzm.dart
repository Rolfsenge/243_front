...
import 'package:gtm/views/detail_secteur.dart';
import 'package:http/http.dart' as http;

import '../api_connection/api_connection.dart';
import '../class/utilisateur.dart';
import '../tl/detail_secteur_tl.dart';
import '../tools/color.dart';
import 'detail_zone_page.dart';
import 'zone_detail_pdv.dart';
import 'kpi_zone.dart';

class HomeRzm extends StatefulWidget {
  final Utilisateur utilisateur;
  const HomeRzm({super.key, required this.utilisateur});

  @override
  State<HomeRzm> createState() => _HomeRzmState();
}

class _HomeRzmState extends State<HomeRzm> {
  ...
  Future<Map<String, dynamic>> fetchDashboard() async {
    final response = await http.post(
      Uri.parse(API.getAcquisitionZone),
                                ...
                                if (index == 0) {
                                  Get.to(
                                    () => DetailZonePage(
                                      ...
                                      color: Colors.green,
                                      zone: widget.utilisateur.nomstructure,
                                      data: revenus,
                                    ),
                                  );
                                } else if (index == 1) {
          ...
          onTap: () {
            Get.to(
              () => KpiZone(
                utilisateur: widget.utilisateur,
                kpi: item['kpi'],
                mtd_zone: item['total_mtd'],
                pmtd_zone: item['total_pmtd'],
                zone: item['zone'],
              ),
            );
          },
  ...
  Widget _buildCanalTlCard(Map<String, dynamic> region) {
    final zoneName = _str(region['zone']);
    ...
    return InkWell(
      onTap: () {
        Get.to(
          () =>
              DetailSecteurTl(utilisateur: widget.utilisateur, zone: zoneName),
        );
      },
      child: Card(
        elevation: 20,
