...
class ResetKabuPage extends StatefulWidget {
  final Utilisateur utilisateur;
  const ResetKabuPage({super.key, required this.utilisateur});

  @override
  State<ResetKabuPage> createState() => _ResetKabuPageState();
}

class _ResetKabuPageState extends State<ResetKabuPage> {
  ...
  Future<void> _saveResetKaabu() async {
    if (_formKey.currentState!.validate()) {
      Map<String, dynamic> data = {
        'resetkaabu_idutilisateur': widget.utilisateur.idutilisateur,
        'numero': numeroCtrl.text,
        'login': loginCtrl.text,
      };

      await helper.inserer(data,'resetkaabu');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Reset Pin Kaabu", style: TextStyle(color: whiteColor),),
            ...
            children: [
              ...
              TextFormField(
                ...
                inputFormatters: [
                  ...
                  LengthLimitingTextInputFormatter(9),    // ✅ limite à 9 caractères
                ],
                decoration: const InputDecoration(
                  labelText: "Numéro de téléphone",
                  prefixIcon: Icon(Icons.phone_android),// masque le compteur visuel
                ),
                validator: (value) {
                  print(value);
