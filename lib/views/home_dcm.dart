...
import 'package:gtm/class/secteur.dart';
import 'package:gtm/tl/detail_secteur_tl.dart';
import 'package:http/http.dart' as http;

import '../api_connection/api_connection.dart';
import '../class/utilisateur.dart';
import '../tl/detail_acq_tl.dart';
import '../tools/color.dart';
import 'detail_secteur.dart';
import 'kpi_secteur.dart';
import 'pdv_detail_secteur.dart';

class HomeDcm extends StatefulWidget {
  final Utilisateur utilisateur;
  const HomeDcm({super.key, required this.utilisateur});

  @override
  State<HomeDcm> createState() => _HomeDcmState();
}

class _HomeDcmState extends State<HomeDcm> {
  ...
  Future<Map<String, dynamic>> fetchDashboard() async {
    final response = await http.post(
      Uri.parse(API.getAcquisitionSecteur),
                        ...
                        child: PieChart(
                          PieChartData(
                            ...
                            sections: [
                              ...
                              PieChartSectionData(
                                value: _toDouble(revenus['surperformer']),
                                color: Colors.blue,
                                title: '${_str(revenus['taux_superformer'])}%',
                                radius: 70,
                                titleStyle: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 13,
                            ...
                            // ✅ Seuls les non rentables sont cliquables
                                ...
                                if (index == 1) {
                                  Get.to(
                                    () => DetailSecteur(
                                      ...
                                      color: primaryColor,
                                      secteur: widget.utilisateur.nomstructure,
                                      data: revenus,
                                    ),
                                  );
                                }
                      ...
                      _Legend(
                        entries: [
                          ...
                          LegendEntry(
                            color: Colors.blue,
                            label: "Surperformer",
                            value: _str(revenus['surperformer']),
                      ...
                      SizedBox(
                        ...
                        child: BarChart(
                          BarChartData(
                            ...
                            // ✅ Seuls les PDV en rupture sont cliquables
      ...
      return Card(
        ...
        child: InkWell(
          onTap: () {
            Get.to(
              () => KpiSecteur(
                utilisateur: widget.utilisateur,
                kpi: item['kpi'],
                mtd_secteur: item['total_mtd'],
                pmtd_secteur: item['total_pmtd'],
                secteur: widget.utilisateur.nomstructure,
              ),
            );
          },
          child: ListTile(
            ...
            subtitle: Text(
              'MTD : ${item['total_mtd']} | PMTD : ${item['total_pmtd']}',
              style: TextStyle(
                color: blackColor,
  ...
  Widget _buildCanalTlCard(Map<String, dynamic> region) {
    final secteurName = _str(region['secteur']);
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Text(
          '${entry.label}: ${entry.value} (${entry.percent ?? '0'}%)',
