...
import 'revenu_site_zone.dart';

class HomeRegion extends StatefulWidget {
  final Utilisateur utilisateur;
  const HomeRegion({super.key, required this.utilisateur});

  @override
  State<HomeRegion> createState() => _HomeRegionState();
}

class _HomeRegionState extends State<HomeRegion> {
  ...
  Future<Map<String, dynamic>> fetchDashboard() async {
    final response = await http.post(
      Uri.parse(API.getAcquisitionRegion),
      body: {'idregion': widget.utilisateur.idstructure.toString()},
    );
                                ...
                                if (index == 0) {
                                  Get.to(
                                    () => RevenuSiteZone(
                                      ...
                                      color: Colors.green,
                                      total: revenus['total_sites'].toString(),
          ...
          onTap: () {
            Get.to(
              () => KpiRegion(
                utilisateur: widget.utilisateur,
                kpi: item['kpi'],
                mtd_region: item['total_mtd'],
                pmtd_region: item['total_pmtd'],
                region: item['region'],
              ),
            );
          },
